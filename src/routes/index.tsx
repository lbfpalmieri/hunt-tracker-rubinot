...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Acompanhe suas hunts no RubinOT: XP/h, lucro/h, monstros por hora e evolução." },
      { property: "og:title", content: "RubinOT Hunt Tracker" },
      { property: "og:description", content: "Ferramenta para monitorar hunts, lucro e XP no RubinOT." },
      ...
      {!hydrated ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard
              label="Lucro / hora (média)"
              value={fmtGold(agg.gph)}
              hint="gold médio por hora"
              icon={Coins}
              accent="gold"
            />
            <StatCard
              label="Tempo total"
              value={fmtDuration(agg.totalTime)}
              hint={`${mySessions.length} sessões`}
              icon={Timer}
              accent="muted"
            />
            <StatCard
              ...
              hint={agg.bestHunt ? `Top spot: ${agg.bestHunt.name}` : ""}
              icon={Trophy}
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="card-surface p-5">
              <div className="mb-4 flex items-center justify-between">
                ...
                <Link to="/sessions" className="text-xs text-rubi-blue hover:underline">
                  ver todas
                </Link>
              </div>
                ...
                {mySessions.slice(0, 6).map((s) => (
                  <li key={s.id}>
                    <Link
                      to="/sessions/$id"
                      params={{ id: s.id }}
                      className="flex items-center justify-between rounded-lg border border-transparent px-3 py-2 text-sm transition-colors hover:border-border hover:bg-accent/40"
                    >
                      ...
                      <div className="text-right">
                        <div
                          className={
                            ...
                            (s.hunting.balance >= 0 ? "text-rubi-success" : "text-rubi-danger")
                          }
                        >
                          {fmtGold(s.hunting.balance)}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {fmtNum(s.hunting.xpPerHour)} xp/h
                        </div>
                      </div>
                    </Link>
                  </li>
